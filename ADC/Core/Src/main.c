...
int main(void)
{
  ...
  while (1)
  {
      // ADC Okumayı Başlat ve Değeri Al
      HAL_ADC_Start(&hadc1);
      ...
      // 1. Voltaj ve Sıcaklık Hesapla
      voltaj_degeri = (adc_ham_deger * 3.3f) / 4095.0f;
      float temp_C = voltaj_degeri * 100.0f;
      float temp_F = (temp_C * 1.8f) + 32.0f;

      // 2. LCD Görüntüleme
      LCD_SetCursor(0, 0);
      sprintf(lcd_buffer, "Voltage: %.2fV  ", voltaj_degeri);
      LCD_Print(lcd_buffer);

      LCD_SetCursor(1, 0);
      // SW0 (PE13) kontrolü [cite: 49, 50]
      if(HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_13) == GPIO_PIN_RESET) {
          sprintf(lcd_buffer, "Temp: %.1f C    ", temp_C);
      } else {
          sprintf(lcd_buffer, "Temp: %.1f F    ", temp_F);
      }
      LCD_Print(lcd_buffer);

      // 3. LED Kademe Kontrolü (SW1 - PE14) [cite: 52, 57]
      if(HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_14) == GPIO_PIN_SET) {
          int kademe = (int)(temp_C / 12.5f);
          if(kademe > 8) kademe = 8;

          for(int i = 0; i < 8; i++) {
              // Sol baştan (LED7'den başlayarak) i. kademeye kadar yak [cite: 53]
              if(i < kademe)
          ...
          // SW1 kapalıysa tüm LED'leri söndür [cite: 57]
          HAL_GPIO_WritePin(GPIOE, GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3|
                                   GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7, GPIO_PIN_RESET);
      }

      HAL_Delay(200);
            MX_USB_HOST_Process(); // Bu fonksiyon döngü içinde olmalı
        }
        /* USER CODE END WHILE */
    MX_USB_HOST_Process();
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOD, LD4_Pin|LD3_Pin|LD5_Pin|LD6_Pin
                          |GPIO_PIN_2|GPIO_PIN_3|Audio_RST_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : LED_2_Pin LED_3_Pin LED_4_Pin LED_5_Pin
                           LED_6_Pin LED_7_Pin LED_0_Pin LED_1_Pin */
  ...
  /*Configure GPIO pins : LD4_Pin LD3_Pin LD5_Pin LD6_Pin
                           PD2 PD3 Audio_RST_Pin */
  GPIO_InitStruct.Pin = LD4_Pin|LD3_Pin|LD5_Pin|LD6_Pin
                          |GPIO_PIN_2|GPIO_PIN_3|Audio_RST_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
